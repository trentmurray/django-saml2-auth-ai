...
def _merge_dict(d1, d2):
    for k, v2 in d2.items():
        v1 = d1.get(k)
        if (
            isinstance(v1, collections.Mapping) and
...
def welcome(r):
    try:
        ...
        return HttpResponseRedirect(settings.SAML2_AUTH.get('DEFAULT_NEXT_URL',
                                    get_reverse('admin:index')))
...
def _create_new_user(username, email, firstname, lastname):
    ...
    user.is_staff = settings.SAML2_AUTH.get(
        'NEW_USER_PROFILE', {}).get('STAFF_STATUS', False)
...
def acs(r):
    ...
    if not resp:
        return HttpResponseRedirect(get_reverse([denied, 'denied',
    ...
    try:
        ...
        if settings.SAML2_AUTH.get('TRIGGER', {}).get('BEFORE_LOGIN', None):
            import_string(settings.SAML2_AUTH[
                'TRIGGER']['BEFORE_LOGIN'])(user_identity)
    ...
    if is_new_user:
        try:
            return render(
                r, 'django_saml2_auth/welcome.html', {'user': r.user})
        except TemplateDoesNotExist:
...
def signin(r):
    ...
    idp_entity_id = None
    try:
        client_settings = settings.SAML2_AUTH['SAML_CLIENT_SETTINGS']
        idp_entity_id = client_settings['service']['sp']['idp']
    except KeyError:
        pass
